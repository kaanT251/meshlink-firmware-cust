...
void save_crypto_to_file(const char *filename) {
  File32 cryptoFile = fatfs.open(filename, FILE_WRITE);
  if (!cryptoFile) {
    LOG_ERROR("Failed to open crypto file %s for writing", filename);
  } else {
    nRFCrypto.begin();
    nRFCrypto.Random.generate(buffer, sizeof(buffer));
    for (int i = 0; i < sizeof(buffer); i++) {
      cryptoFile.print(buffer[i]);
    }
    cryptoFile.print('\n');
    cryptoFile.close();
    cryptoFile = fatfs.open(filename);
  }

  if (!cryptoFile) {
    LOG_ERROR("Failed to open crypto file %s for reading", filename);
  } else {
    while (cryptoFile.available()) {
      LOG_INFO("File output: %d", cryptoFile.read());
    }
    cryptoFile.close();
  }
}
