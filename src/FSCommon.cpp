...
bool write_data(pb_ostream_t *stream, const pb_field_t *field,
                void *const *arg) {
  return pb_encode_tag_for_field(stream, field) &&
         pb_encode_string(stream, (uint8_t *)(*arg), 32);
}

bool read_data(pb_istream_t *stream, const pb_field_t *field, void **arg) {
  // arg points to your buffer (uint8_t[32])
  return pb_read(stream, (uint8_t *)(*arg), 32);
}
bool save_crypto_to_file(const char *filename) {
  uint8_t buffer[32];
  uint8_t buffer_read[32];
  nrfCrypto.begin();
  nrfCrypto.Random.generate(buffer, sizeof(buffer));

  meshlink_Buffer32 buf = {};
  buf.data.funcs.encode = &write_data;
  buf.data.arg = buffer;

  nodeDB->saveProto(filename, sizeof(meshlink_Buffer32), &meshlink_Buffer32_msg,
                    &buf, true);

  buf.data.funcs.decode = &read_data;
  buf.data.arg = buffer_read;
  if (nodeDB->loadProto(filename, sizeof(meshlink_Buffer32),
                        sizeof(meshlink_Buffer32), &meshlink_Buffer32_msg,
                        &buf)) {
    LOG_INFO("Loaded Buffer32:");
    for (int i = 0; i < 32; ++i) {
      LOG_INFO("raw[%d] = 0x%02X", i, buffer_read[i]);
    }
  } else {
    LOG_ERROR("Failed to load crypto.pb");
    return false;
  }
  return true;
}
...
static void buildPath(char *dest, size_t destLen, const char *parent,
                      const char *child) {
  if (strcmp(parent, "/") == 0) {
...
#ifdef USE_EXTERNAL_FLASH
  // Adafruit SPI Flash FatFs implementation does not support recursive delete,
