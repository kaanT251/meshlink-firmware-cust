...
meshtastic_DeviceUIConfig uiconfig{.screen_brightness = 153,
                                   .screen_timeout = 30};
...
bool nanopb_fatfs_write(pb_ostream_t *stream, const pb_byte_t *buf,
                        size_t count) {
  File32 *file = (File32 *)stream->state;
...
uint32_t read8(uint8_t bits, uint8_t dummy, uint8_t cs, uint8_t sck,
               uint8_t mosi, uint8_t dc, uint8_t rst) {
  uint32_t ret = 0;
  uint8_t SDAPIN = mosi;
...
void write9(uint8_t val, uint8_t dc_val, uint8_t cs, uint8_t sck, uint8_t mosi,
...
uint32_t readwrite8(uint8_t cmd, uint8_t bits, uint8_t dummy, uint8_t cs,
                    uint8_t sck, uint8_t mosi, uint8_t dc, uint8_t rst) {
  digitalWrite(cs, LOW);
...
uint32_t get_st7789_id(uint8_t cs, uint8_t sck, uint8_t mosi, uint8_t dc,
                       uint8_t rst) {
  pinMode(cs, OUTPUT);
  digitalWrite(cs, HIGH);
  pinMode(cs, OUTPUT);
...
bool meshtastic_NodeDatabase_callback(pb_istream_t *istream,
  ...
  if (ostream) {
    std::vector<meshtastic_NodeInfoLite> const *vec =
        (std::vector<meshtastic_NodeInfoLite> *)field->pData;
    for (auto item : *vec) {
...
/** The current change # for radio settings.  Starts at 0 on boot and any time
 * the radio settings might have changed is incremented.  Allows others to
 * detect they might now be on a new channel.
 */
...
/**
 *
 * Normally userids are unique and start with +country code to look like Signal
 * phone numbers. But there are some special ids used when we haven't yet been
 * configured by a user.  In that case we use !macaddr (no colons).
 */
meshtastic_User &owner = devicestate.owner;
meshtastic_Position localPosition = meshtastic_Position_init_default;
meshtastic_CriticalErrorCode error_code =
    meshtastic_CriticalErrorCode_NONE; // For the error code, only show values
                                       // from this boot (discard value from
                                       // flash)
...
NodeDB::NodeDB() {
  ...
  // ESP32 factory burns a unique id in efuse for S2+ series and evidently C3+
  // series This is used for HMACs in the esp-rainmaker AIOT platform and seems
  // to be a good choice for us
  esp_err_t err = esp_efuse_read_field_blob(ESP_EFUSE_OPTIONAL_UNIQUE_ID,
                                            unique_id, sizeof(unique_id) * 8);
  if (err == ESP_OK) {
  ...
  // We concatenate the device address to the Random ID to create a unique ID
  // for now This will likely utilize a crypto module in the future
  uint64_t device_id_start =
      ((uint64_t)NRF_FICR->DEVICEID[1] << 32) | NRF_FICR->DEVICEID[0];
  uint64_t device_id_end =
      ((uint64_t)NRF_FICR->DEVICEADDR[1] << 32) | NRF_FICR->DEVICEADDR[0];
  memcpy(myNodeInfo.device_id.bytes, &device_id_start, sizeof(device_id_start));
  memcpy(myNodeInfo.device_id.bytes + sizeof(device_id_start), &device_id_end,
         sizeof(device_id_end));
  myNodeInfo.device_id.size = 16;
  ...
  // likewise - we always want the app requirements to come from the running
  // appload
  myNodeInfo.min_app_version =
      30200; // format is Mmmss (where M is 1+the numeric major number. i.e.
             // 30200 means 2.2.00
  // Note! We do this after loading saved settings, so that if somehow an
  // invalid nodenum was stored in preferences we won't keep using that nodenum
  // forever. Crummy guess at our nodenum (but we will check against the nodedb
  // to avoid conflicts)
  pickNewNodeNum();
  ...
  if (config.security.private_key.size == 32 &&
    ...
    memcpy(owner.public_key.bytes, config.security.public_key.bytes,
           config.security.public_key.size);
  ...
  // nodeDB->LOG_DEBUG("region=%d, NODENUM=0x%x, dbsize=%d", config.lora.region,
  // myNodeInfo.my_node_num, numMeshNodes);
  ...
  // If we are setup to broadcast on the default channel, ensure that the
  // telemetry intervals are coerced to the minimum value of 30 minutes or more
  if (channels.isDefaultChannel(channels.getPrimaryIndex())) {
    ...
    moduleConfig.telemetry.device_update_interval =
        Default::getConfiguredOrMinimumValue(
            moduleConfig.telemetry.device_update_interval,
            min_default_telemetry_interval_secs);
  ...
  if (moduleConfig.mqtt.has_map_report_settings &&
      moduleConfig.mqtt.map_report_settings.publish_interval_secs <
  ...
  moduleConfig.neighbor_info.update_interval =
      Default::getConfiguredOrMinimumValue(
          moduleConfig.neighbor_info.update_interval,
          min_neighbor_info_broadcast_secs);
  ...
  if (config.display.displaymode ==
    ...
    // On a device without MUI, this display mode makes no sense, and will break
    // logic.
  ...
  if (myNodeInfo.reboot_count ==
      1) { // Check if First boot ever or after Factory Reset.
...
/**
 * Most (but not always) of the time we want to treat packets 'from' the local
 * phone (where from == 0), as if they originated on the local node. If from is
 * zero this function returns our node number instead
 */
...
bool NodeDB::factoryReset(bool eraseBleBonds) {
  ...
  if (FSCom.exists("/static/rangetest.csv") &&
      !FSCom.remove("/static/rangetest.csv")) {
    LOG_ERROR("Could not remove rangetest.csv file");
  }
#endif
  spiLock->unlock();
  // second, install default state (this will deal with the duplicate mac
  // address issue)
  installDefaultNodeDatabase();
  installDefaultDeviceState();
  installDefaultConfig(!eraseBleBonds); // Also preserve the private key if
                                        // we're not erasing BLE bonds
...
#ifdef ARCH_ESP32
    // This will erase what's in NVS including ssl keys, persistent variables
    // and ble pairing
    nvs_flash_erase();
#endif
#ifdef ARCH_NRF52
...
void NodeDB::installDefaultConfig(bool preserveKey = false) {
  ...
  bool shouldPreserveKey = preserveKey && config.has_security &&
                           config.security.private_key.size > 0;
  if (shouldPreserveKey) {
    memcpy(private_key_temp, config.security.private_key.bytes,
  ...
  config.lora.tx_enabled =
      true; // FIXME: maybe false in the future, and setting region to enable
            // it. (unset region forces it off)
...
#else
  config.device.role =
      meshtastic_Config_DeviceConfig_Role_CLIENT; // Default to client.
#endif
  ...
  if (shouldPreserveKey) {
    config.security.private_key.size = 32;
    memcpy(config.security.private_key.bytes, private_key_temp,
           config.security.private_key.size);
  ...
  resetRadioConfig(
      true); // This also triggers NodeInfo/Position requests since we're fresh
  strncpy(config.network.ntp_server, "meshtastic.pool.ntp.org", 32);

#if (defined(T_DECK) || defined(T_WATCH_S3) || defined(UNPHONE) ||             \
     defined(PICOMPUTER_S3) || defined(SENSECAP_INDICATOR) ||                  \
  ...
  config.bluetooth.fixed_pin = defaultBLEPin;
    ...
    defined(ILI9342_DRIVER) || defined(ST7789_CS) || defined(HX8357_CS) ||     \
    defined(USE_ST7789) || defined(ILI9488_CS) || defined(ST7796_CS) ||        \
    defined(USE_SPISSD1306)
  bool hasScreen = true;
#ifdef HELTEC_MESH_NODE_T114
  uint32_t st7789_id = get_st7789_id(ST7789_NSS, ST7789_SCK, ST7789_SDA,
                                     ST7789_RS, ST7789_RESET);
...
// Set default value for 'Mesh via UDP'
  ...
  strncpy(config.network.wifi_ssid, USERPREFS_NETWORK_WIFI_SSID,
          sizeof(config.network.wifi_ssid));
#endif

#ifdef USERPREFS_NETWORK_WIFI_PSK
  strncpy(config.network.wifi_psk, USERPREFS_NETWORK_WIFI_PSK,
          sizeof(config.network.wifi_psk));
#endif
...
void NodeDB::installDefaultModuleConfig() {
...
#ifdef HAS_I2S
  // Don't worry about the other settings for T-Watch, we'll also use the
  // DRV2056 behavior for notifications
  moduleConfig.external_notification.enabled = true;
  moduleConfig.external_notification.use_i2s_as_buzzer = true;
  moduleConfig.external_notification.alert_message_buzzer = true;
#if HAS_TFT
  if (moduleConfig.external_notification.nag_timeout ==
      default_ringtone_nag_secs)
  ...
  strncpy(moduleConfig.mqtt.address, USERPREFS_MQTT_ADDRESS,
          sizeof(moduleConfig.mqtt.address));
#else
  strncpy(moduleConfig.mqtt.address, default_mqtt_address,
          sizeof(moduleConfig.mqtt.address));
#endif
#ifdef USERPREFS_MQTT_USERNAME
  strncpy(moduleConfig.mqtt.username, USERPREFS_MQTT_USERNAME,
          sizeof(moduleConfig.mqtt.username));
#else
  strncpy(moduleConfig.mqtt.username, default_mqtt_username,
          sizeof(moduleConfig.mqtt.username));
#endif
#ifdef USERPREFS_MQTT_PASSWORD
  strncpy(moduleConfig.mqtt.password, USERPREFS_MQTT_PASSWORD,
          sizeof(moduleConfig.mqtt.password));
#else
  strncpy(moduleConfig.mqtt.password, default_mqtt_password,
          sizeof(moduleConfig.mqtt.password));
#endif
#ifdef USERPREFS_MQTT_ROOT_TOPIC
  strncpy(moduleConfig.mqtt.root, USERPREFS_MQTT_ROOT_TOPIC,
          sizeof(moduleConfig.mqtt.root));
#else
  strncpy(moduleConfig.mqtt.root, default_mqtt_root,
          sizeof(moduleConfig.mqtt.root));
#endif
  ...
  moduleConfig.ambient_lighting.green =
      (myNodeInfo.my_node_num & 0x00FF00) >> 8;
...
void NodeDB::installRoleDefaults(meshtastic_Config_DeviceConfig_Role role) {
  if (role == meshtastic_Config_DeviceConfig_Role_ROUTER) {
    ...
    // Remove Altitude MSL from flags since CoTs use HAE (height above
    // ellipsoid)
    config.position.position_flags =
        (meshtastic_Config_PositionConfig_PositionFlags_ALTITUDE |
         meshtastic_Config_PositionConfig_PositionFlags_SPEED |
...
void NodeDB::initModuleConfigIntervals() {
  ...
  moduleConfig.telemetry.device_update_interval =
      USERPREFS_CONFIG_DEVICE_TELEM_UPDATE_INTERVAL;
#else
  moduleConfig.telemetry.device_update_interval = MAX_INTERVAL;
#endif
...
void NodeDB::resetNodes(bool keepFavorites) {
  ...
  if (keepFavorites) {
    ...
    std::fill(nodeDatabase.nodes.begin() + 1, nodeDatabase.nodes.end(),
...
void NodeDB::removeNodeByNum(NodeNum nodeNum) {
  ...
  std::fill(nodeDatabase.nodes.begin() + numMeshNodes,
            nodeDatabase.nodes.begin() + numMeshNodes + 1,
            meshtastic_NodeInfoLite());
  ...
  for (int i = 0; i < numMeshNodes; i++) {
    if (meshNodes->at(i).has_user) {
      if (meshNodes->at(i).user.public_key.size > 0) {
        if (memfll(meshNodes->at(i).user.public_key.bytes, 0,
                   meshNodes->at(i).user.public_key.size)) {
...
void NodeDB::installDefaultDeviceState() {
  ...
  snprintf(owner.long_name, sizeof(owner.long_name),
           (const char *)USERPREFS_CONFIG_OWNER_LONG_NAME);
#else
  snprintf(owner.long_name, sizeof(owner.long_name), "Meshtastic %04x",
           getNodeNum() & 0x0ffff);
#endif
#ifdef USERPREFS_CONFIG_OWNER_SHORT_NAME
  snprintf(owner.short_name, sizeof(owner.short_name),
           (const char *)USERPREFS_CONFIG_OWNER_SHORT_NAME);
#else
  snprintf(owner.short_name, sizeof(owner.short_name), "%04x",
           getNodeNum() & 0x0ffff);
#endif
  snprintf(owner.id, sizeof(owner.id), "!%08x",
           getNodeNum()); // Default node ID now based on nodenum
  memcpy(owner.macaddr, ourMacAddr, sizeof(owner.macaddr));
  owner.has_is_unmessagable = true;
...
void NodeDB::pickNewNodeNum() {
  ...
  if (nodeNum == 0) {
    ...
    nodeNum = (ourMacAddr[2] << 24) | (ourMacAddr[3] << 16) |
              (ourMacAddr[4] << 8) | ourMacAddr[5];
  }

  meshtastic_NodeInfoLite *found;
  while (((found = getMeshNode(nodeNum)) &&
          memcmp(found->user.macaddr, ourMacAddr, sizeof(ourMacAddr)) != 0) ||
         (nodeNum == NODENUM_BROADCAST || nodeNum < NUM_RESERVED)) {
    NodeNum candidate =
        random(NUM_RESERVED, LONG_MAX); // try a new random choice
    if (found)
      LOG_WARN("NOTE! Our desired nodenum 0x%x is invalid or in use, by MAC "
               "ending in 0x%02x%02x vs our 0x%02x%02x, so "
               "trying for 0x%x",
               nodeNum, found->user.macaddr[4], found->user.macaddr[5],
               ourMacAddr[4], ourMacAddr[5], candidate);
    nodeNum = candidate;
  }
...
LoadFileResult NodeDB::loadProto(const char *filename, size_t protoSize,
                                 size_t objSize, const pb_msgdesc_t *fields,
                                 void *dest_struct) {
  ...
  if (f) {
    ...
    if (fields != &meshtastic_NodeDatabase_msg) { // special case for NodeDB
    ...
    */
    ...
    /*
     ...
     if (got <= 0) {
         LOG_ERROR("Error reading file: read %d bytes at offset %u", got,
 (unsigned)totalRead); break;
     }
     totalRead += got;
 }
     ...
     //if (got != int(objSize)) {
         LOG_ERROR("Error reading file: read %u of %u bytes",
 (unsigned)totalRead, (unsigned)objSize);
         //LOG_ERROR("Error reading file: read %d of %u bytes", got,
 (unsigned)objSize); state = LoadFileResult::DECODE_FAILED; } else {
...
void NodeDB::loadFromDisk() {
  // Mark the current device state as completely unusable, so that if we fail
  // reading the entire file from disk we will still factoryReset to restore
  // things.
  devicestate.version = 0;

  meshtastic_Config_SecurityConfig backupSecurity =
      meshtastic_Config_SecurityConfig_init_zero;

#ifdef ARCH_ESP32
  spiLock->lock();
  // If the legacy deviceState exists, start over with a factory reset
  if (FSCom.exists("/static/static"))
    rmDir("/static/static"); // Remove bad static web files bundle from
                             // initial 2.5.13 release
  spiLock->unlock();
#endif
#ifdef FSCom
  ...
  if (FSCom.exists(legacyPrefFileName)) {
    ...
    if (loadProto(configFileName, meshtastic_LocalConfig_size,
                  sizeof(meshtastic_LocalConfig), &meshtastic_LocalConfig_msg,
...
#endif
  auto state = loadProto(nodeDatabaseFileName, getMaxNodesAllocatedSize(),
                         sizeof(meshtastic_NodeDatabase),
  ...
  if (nodeDatabase.version < DEVICESTATE_MIN_VER) {
    ...
    LOG_INFO("Loaded saved nodedatabase version %d, with nodes count: %d",
             nodeDatabase.version, nodeDatabase.nodes.size());
  }

  if (numMeshNodes > MAX_NUM_NODES) {
    LOG_WARN("Node count %d exceeds MAX_NUM_NODES %d, truncating", numMeshNodes,
             MAX_NUM_NODES);
  ...
  state = loadProto(deviceStateFileName, meshtastic_DeviceState_size,
                    sizeof(meshtastic_DeviceState), &meshtastic_DeviceState_msg,
                    &devicestate);

  // See
  // https://github.com/meshtastic/firmware/issues/4184#issuecomment-2269390786
  // It is very important to try and use the saved prefs even if we fail to read
  // meshtastic_DeviceState.  Because most of our critical config may still be
  // valid (in the other files - loaded next). Also, if we did fail on reading
  // we probably failed on the enormous (and non critical) nodeDB.  So DO NOT
  // install default device state. if (state != LoadFileResult::LOAD_SUCCESS) {
  //    installDefaultDeviceState(); // Our in RAM copy might now be corrupt
  //} else {
  if ((state != LoadFileResult::LOAD_SUCCESS) ||
      (devicestate.version < DEVICESTATE_MIN_VER)) {
  ...
  state = loadProto(configFileName, meshtastic_LocalConfig_size,
                    sizeof(meshtastic_LocalConfig), &meshtastic_LocalConfig_msg,
                    &config);
  if (state != LoadFileResult::LOAD_SUCCESS) {
  ...
  // Make sure we load hard coded admin keys even when the configuration file
  // has none. Initialize admin_key_count to zero
  byte numAdminKeys = 0;
#if defined(USERPREFS_USE_ADMIN_KEY_0) ||                                      \
    defined(USERPREFS_USE_ADMIN_KEY_1) || defined(USERPREFS_USE_ADMIN_KEY_2)
  uint16_t sum = 0;
#endif
#ifdef USERPREFS_USE_ADMIN_KEY_0
  ...
  state = loadProto(moduleConfigFileName, meshtastic_LocalModuleConfig_size,
                    sizeof(meshtastic_LocalModuleConfig),
  ...
  state = loadProto(channelFileName, meshtastic_ChannelFile_size,
  ...
  state = loadProto(uiconfigFileName, meshtastic_DeviceUIConfig_size,
                    sizeof(meshtastic_DeviceUIConfig),
  ...
  if (moduleConfig.version < 23) {
    LOG_DEBUG(
        "ModuleConfig version %d is stale, upgrading to new default intervals",
  ...
  if (portduino_config.has_configDisplayMode) {
    config.display.displaymode = (_meshtastic_Config_DisplayConfig_DisplayMode)
                                     portduino_config.configDisplayMode;
  }
...
bool NodeDB::saveProto(const char *filename, size_t protoSize,
                       const pb_msgdesc_t *fields, const void *dest_struct,
                       bool fullAtomic) {
  bool okay = false;
#ifdef USE_EXTERNAL_FLASH
  ...
  if (!f) {
    ...
    /*size_t written = f.write(reinterpret_cast<const uint8_t*>(dest_struct),
    protoSize);
     ...
     // --- Streaming write implementation ---
      ...
      while (totalWritten < protoSize) {
         ...
         int written = f.write(src + totalWritten, toWrite);
          if (written <= 0) {
             LOG_ERROR("Error writing file: wrote %d bytes at offset %u",
   written, (unsigned)totalWritten); break;
          }
     ...
     if (totalWritten != protoSize) {
         LOG_ERROR("Error: wrote %u of %u bytes", (unsigned)totalWritten,
   (unsigned)protoSize); okay = false; } else { LOG_INFO("File saved!");
         LOG_INFO("File closed!");
         okay = true;
     }
   }
...
bool NodeDB::saveChannelsToDisk() {
  ...
  spiLock->unlock();
#endif
  return saveProto(channelFileName, meshtastic_ChannelFile_size,
...
bool NodeDB::saveDeviceStateToDisk() {
  ...
  spiLock->unlock();
#endif
  // Note: if MAX_NUM_NODES=100 and meshtastic_NodeInfoLite_size=166, so will be
  // approximately 17KB Because so huge we _must_ not use fullAtomic, because
  // the filesystem is probably too small to hold two copies of this
  return saveProto(deviceStateFileName, meshtastic_DeviceState_size,
                   &meshtastic_DeviceState_msg, &devicestate, true);
}
...
bool NodeDB::saveNodeDatabaseToDisk() {
  ...
  pb_get_encoded_size(&nodeDatabaseSize, meshtastic_NodeDatabase_fields,
                      &nodeDatabase);
  return saveProto(nodeDatabaseFileName, nodeDatabaseSize,
                   &meshtastic_NodeDatabase_msg, &nodeDatabase, false);
}
...
bool NodeDB::saveToDiskNoRetry(int saveWhat) {
  ...
  if (saveWhat & SEGMENT_CONFIG) {
    ...
    success &= saveProto(configFileName, meshtastic_LocalConfig_size,
  ...
  if (saveWhat & SEGMENT_MODULECONFIG) {
    ...
    success &=
        saveProto(moduleConfigFileName, meshtastic_LocalModuleConfig_size,
                  &meshtastic_LocalModuleConfig_msg, &moduleConfig);
  }
  ...
  if (!flash.begin()) {
    ...
    LOG_INFO("File created!");
     ...
     if (dataFile) {
       ...
       config.device.role = meshtastic_Config_DeviceConfig_Role_CLIENT;  //
  using proper enum config.lora.region =
  meshtastic_Config_LoRaConfig_RegionCode_US; config.lora.modem_preset =
  meshtastic_Config_LoRaConfig_ModemPreset_LONG_FAST;
       strncpy(config.network.ntp_server, "",
  sizeof(config.network.ntp_server)-1); config.display.screen_on_secs = 0;
       config.position.gps_mode =
  meshtastic_Config_PositionConfig_GpsMode_NOT_PRESENT;;
       config.security.private_key.size = 0;
         ...
         LOG_INFO("security.private_key.size=%d",
  config.security.private_key.size);
       ...
       if (got == (int)want) {
         ...
         LOG_INFO("position.gps_mode=%d", config.position.gps_mode);
         LOG_INFO("security.private_key.size=%d",
  config.security.private_key.size); if (config.security.private_key.size > 0) {
           char hexbuf[65] = {0};
           size_t n = config.security.private_key.size;
           if (n > 32) n = 32;
           for (size_t i = 0; i < n; ++i)
             sprintf(hexbuf + strlen(hexbuf), "%02x",
  config.security.private_key.bytes[i]); LOG_INFO("security.private_key=%s",
  hexbuf);
         }
       } else {
         LOG_ERROR("Error reading file: read %d of %u bytes", got,
...
bool NodeDB::saveToDisk(int saveWhat) {
  ...
  if (!success) {
...
#ifdef ARCH_NRF52 // @geeksville is not ready yet to say we should do this on
                  // other platforms.  See bug #4184 discussion
    spiLock->lock();
    FSCom.format();
    spiLock->unlock();
    ...
    RECORD_CRITICALERROR(
        success ? meshtastic_CriticalErrorCode_FLASH_CORRUPTION_RECOVERABLE
...
/// Given a node, return how many seconds in the past (vs now) that we last
/// heard from it
 ...
 */
void NodeDB::updatePosition(uint32_t nodeId, const meshtastic_Position &p,
                            RxSource src) {
  ...
  if (src == RX_SRC_LOCAL) {
    ...
    LOG_INFO("updatePosition LOCAL pos@%x time=%u lat=%d lon=%d alt=%d",
             p.timestamp, p.time, p.latitude_i, p.longitude_i, p.altitude);

    setLocalPosition(p);
    info->position = TypeConversions::ConvertToPositionLite(p);
  } else if ((p.time > 0) && !p.latitude_i && !p.longitude_i && !p.timestamp &&
             !p.location_source) {
    ...
    // A lot of position reports don't have time populated.  In that case, be
    // careful to not blow away the time we recorded based on the packet rxTime
    //
    // FIXME perhaps handle RX_SRC_USER separately?
    LOG_INFO("updatePosition REMOTE node=0x%x time=%u lat=%d lon=%d", nodeId,
  ...
  notifyObservers(
      true); // Force an update whether or not our node counts have changed
}

/** Update telemetry info for this node based on received metrics
 *  We only care about device telemetry here
 */
void NodeDB::updateTelemetry(uint32_t nodeId, const meshtastic_Telemetry &t,
                             RxSource src) {
  meshtastic_NodeInfoLite *info = getOrCreateMeshNode(nodeId);
...
void NodeDB::addFromContact(meshtastic_SharedContact contact) {
  ...
  // saved public key.
  if ((info->bitfield & NODEINFO_BITFIELD_IS_KEY_MANUALLY_VERIFIED_MASK) &&
      !contact.manually_verified) {
    if (contact.user.public_key.size != info->user.public_key.size ||
        memcmp(contact.user.public_key.bytes, info->user.public_key.bytes,
  ...
  if (contact.should_ignore) {
    ...
    // we need to clear the public key and other cruft, in addition to setting
    // the node as ignored
    ...
    /* Clients are sending add_contact before every text message DM (because
     * clients may hold a larger node database with public keys than the radio
     * holds). However, we don't want to update last_heard just because we sent
     * someone a DM!
     */

    /* "Boring old nodes" are the first to be evicted out of the node database
     * when full. This includes a newly-zeroed nodeinfo because it has:
     * !is_favorite && last_heard==0. To keep this from happening when we
     * addFromContact, we set the new node as a favorite, and we leave
     * last_heard alone (even if it's zero).
     */
    if (config.device.role == meshtastic_Config_DeviceConfig_Role_CLIENT_BASE) {
      // Special case for CLIENT_BASE: is_favorite has special meaning, and we
      // don't want to automatically set it without the user doing so
      // deliberately. We don't normally expect users to use a CLIENT_BASE to
      // send DMs or to add contacts, but we should make sure it doesn't
      // auto-favorite in case they do. Instead, as a workaround, we'll set
      // last_heard to now, so that the add_contact node doesn't immediately get
      // evicted.
      info->last_heard = getTime();
    } else {
      // Normal case: set is_favorite to prevent expiration.
      // last_heard will remain as-is (or remain 0 if this entry wasn't in the
      // nodeDB).
      info->is_favorite = true;
    }

    // As the clients will begin sending the contact with DMs, we want to
    // strictly check if the node is manually verified
 ...
 */
bool NodeDB::updateUser(uint32_t nodeId, meshtastic_User &p,
  ...
  if (p.public_key.size == 32 && nodeId != nodeDB->getNodeNum()) {
    ...
    // Alert the user if a remote node is advertising public key that matches
    // our own
  ...
  if (info->user.public_key.size ==
      32) { // if we have a key for this user already, don't overwrite with a
            // new one
    // if the key doesn't match, don't update nodeDB at all.
    if (p.public_key.size != 32 ||
        (memcmp(p.public_key.bytes, info->user.public_key.bytes, 32) != 0)) {
  ...
  // Always ensure user.id is derived from nodeId, regardless of what was
  // received
  snprintf(p.id, sizeof(p.id), "!%08x", nodeId);

  // Both of info->user and p start as filled with zero so I think this is okay
  auto lite = TypeConversions::ConvertToUserLite(p);
  bool changed = memcmp(&info->user, &lite, sizeof(info->user)) ||
  ...
  if (nodeId != getNodeNum())
    info->channel = channelIndex; // Set channel we need to use to reach this
                                  // node (but don't set our own channel)
  LOG_DEBUG("Update changed=%d user %s/%s, id=0x%08x, channel=%d", changed,
            info->user.long_name, info->user.short_name, nodeId, info->channel);
  info->has_user = true;
...
/// we updateGUI and updateGUIforNode if we think our this change is big enough
/// for a redraw
void NodeDB::updateFrom(const meshtastic_MeshPacket &mp) {
  ...
  if (mp.which_payload_variant == meshtastic_MeshPacket_decoded_tag &&
    ...
    if (mp.rx_time) // if the packet has a valid timestamp use it to update our
                    // last_heard
      info->last_heard = mp.rx_time;

    if (mp.rx_snr)
      info->snr =
          mp.rx_snr; // keep the most recent SNR we received for this node.

    info->via_mqtt = mp.via_mqtt; // Store if we received this packet via MQTT

    // If hopStart was set and there wasn't someone messing with the limit in
    // the middle, add hopsAway
...
bool NodeDB::isFromOrToFavoritedNode(const meshtastic_MeshPacket &p) {
  ...
  //   2. exiting early when a favorite is found, or if both from and to have
  //   been seen

  if (p.to == NODENUM_BROADCAST)
    return isFavorite(p.from); // we never store NODENUM_BROADCAST in the DB, so
                               // we only need to check p.from
  ...
  for (int i = 0; i < numMeshNodes; i++) {
    ...
    if (seenFrom && seenTo)
      return false; // we've seen both, and neither is a favorite, so we can
                    // stop searching early

    // Note: if we knew that sortMeshDB was always called after any change to
    // is_favorite, we could exit early after searching all favorited nodes
    // first.
  }
...
void NodeDB::pause_sort(bool paused) { sortingIsPaused = paused; }

void NodeDB::sortMeshDB() {
  if (!sortingIsPaused &&
      (lastSort == 0 || !Throttle::isWithinTimespanMs(lastSort, 1000 * 5))) {
    lastSort = millis();
    bool changed = true;
    while (changed) { // dumb reverse bubble sort, but probably not bad for what
                      // we're doing
      changed = false;
      for (int i = numMeshNodes - 1; i > 0;
           i--) { // lowest case this should examine is i == 1
        if (meshNodes->at(i - 1).num == getNodeNum()) {
          // noop
        } else if (meshNodes->at(i).num ==
                   getNodeNum()) { // in the oddball case our own node num is
                                   // not at location 0, put it there
          // TODO: Look for at(i-1) also matching own node num, and throw the DB
          // in the trash
          std::swap(meshNodes->at(i), meshNodes->at(i - 1));
          changed = true;
        } else if (meshNodes->at(i).is_favorite &&
                   !meshNodes->at(i - 1).is_favorite) {
          std::swap(meshNodes->at(i), meshNodes->at(i - 1));
          changed = true;
        } else if (!meshNodes->at(i).is_favorite &&
                   meshNodes->at(i - 1).is_favorite) {
          // noop
        } else if (meshNodes->at(i).last_heard >
                   meshNodes->at(i - 1).last_heard) {
          std::swap(meshNodes->at(i), meshNodes->at(i - 1));
          changed = true;
        }
      }
...
// returns true if the maximum number of nodes is reached or we are running low
// on memory
bool NodeDB::isFull() {
  return (numMeshNodes >= MAX_NUM_NODES) ||
...
meshtastic_NodeInfoLite *NodeDB::getOrCreateMeshNode(NodeNum n) {
  ...
  if (!lite) {
    if (isFull()) {
      LOG_INFO("Node database full with %i nodes and %u bytes free. Erasing "
               "oldest entry",
    ...
    lite->num = n;
    LOG_INFO("Adding node to database with %i nodes and %u bytes free!",
             numMeshNodes, memGet.getFreeHeap());
  }
...
UserLicenseStatus NodeDB::getLicenseStatus(uint32_t nodeNum) {
  ...
  return info->user.is_licensed ? UserLicenseStatus::Licensed
                                : UserLicenseStatus::NotLicensed;
}

bool NodeDB::checkLowEntropyPublicKey(
    const meshtastic_Config_SecurityConfig_public_key_t &keyToTest) {
  if (keyToTest.size == 32) {
    uint8_t keyHash[32] = {0};
    memcpy(keyHash, keyToTest.bytes, keyToTest.size);
    crypto->hash(keyHash, 32);
    for (uint16_t i = 0;
         i < sizeof(LOW_ENTROPY_HASHES) / sizeof(LOW_ENTROPY_HASHES[0]); i++) {
      if (memcmp(keyHash, LOW_ENTROPY_HASHES[i],
                 sizeof(LOW_ENTROPY_HASHES[0])) == 0) {
        return true;
      }
    }
  }
...
bool NodeDB::backupPreferences(
  ...
  if (location == meshtastic_AdminMessage_BackupLocation_FLASH) {
    meshtastic_BackupPreferences backup =
    ...
    size_t backupSize;
    pb_get_encoded_size(&backupSize, meshtastic_BackupPreferences_fields,
                        &backup);

    spiLock->lock();
    FSCom.mkdir("/backups");
    spiLock->unlock();
    success = saveProto(backupFileName, backupSize,
...
bool NodeDB::restorePreferences(meshtastic_AdminMessage_BackupLocation location,
  ...
  if (location == meshtastic_AdminMessage_BackupLocation_FLASH) {
    ...
    meshtastic_BackupPreferences backup =
        meshtastic_BackupPreferences_init_zero;
    success = loadProto(backupFileName, meshtastic_BackupPreferences_size,
                        sizeof(meshtastic_BackupPreferences),
                        &meshtastic_BackupPreferences_msg, &backup);
    if (success) {
...
void recordCriticalError(meshtastic_CriticalErrorCode code, uint32_t address,
                         const char *filename) {
  if (filename) {
    LOG_ERROR("NOTE! Record critical error %d at %s:%lu", code, filename,
              address);
  } else {
  ...
  // Currently portuino is mostly used for simulation.  Make sure the user
  // notices something really bad happened
#ifdef ARCH_PORTDUINO
